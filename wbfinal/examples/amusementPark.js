...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      horse = loader.loadAsync("../examples/horse.obj");
      horse.then(function(obj){
        obj.scale.set(0.02,0.02,0.02);
        platform_group.add(obj);
        obj.rotateY((2 * i * Math.PI) / num_poles);
        obj.translateX(0.82 * width);
        horses.push(obj);
      });
    }
   ...
   */
   stepWorld(delta, timeOfDay) {
    ...
    for (let i = 0; i < this.num; i++) {
      ...
      if(this.horses[i]){
        let t = (this.time + i*0.1) % 1;
        if (t < 0.1 || t > 0.9) this.horses[i].position.y = .1;
        else{
          this.horses[i].position.y =  .1  + 8 * (0.16 - (0.5 - t) * (0.5 - t));
        }
      }
...
let SpinningTeacupsObCtr = 0;
// A simple merry-go-round.
/**
 * @typedef SpinningTeacupsProperties
 * @type {object}
 * @property {number} [x=0]
 * @property {number} [y=0]
 * @property {number} [z=0]
 * @property {number} [size=1]
 */
export class GrSpinningTeacups extends GrObject {
  /**
   * @param {SpinningTeacupsProperties} params
   */
  constructor(params = {}) {
    let simpleRoundabout = new T.Group();
    ...
    let platform_geom = new T.CylinderGeometry(3, 1.8, 0.3, 8, 4);
    let platform_mat = new T.MeshStandardMaterial({
      color: "pink",
      metalness: 0.3,
      roughness: 0.6
    ...
    for (let i = 0; i < num_cups; i++) {
      let cup = loader.loadAsync("../exmaples/cup.obj");
      cup.then(function(obj){
        obj.scale.set(.5,.5,.5);
        platform_group.add(obj);
        obj.translateY(.5);
        obj.rotateY((2 * i * Math.PI) / num_cups);
        obj.translateX(.5 * width);
        cups.push(obj);
      });
    }
  ...
  stepWorld(delta, timeOfDay) {
    this.platform.rotateY(0.005 * delta);
    this.cups.forEach(function(horse) {
      horse.rotateY(0.01 * delta);
    });
  }
